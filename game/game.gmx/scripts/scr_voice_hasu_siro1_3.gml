...
ds_queue_enqueue(opp_words[? "s3_p2"],"[spr]hasu_bind_siro,0",0.1,"どうだ？　私のが気持ち良いのだろう？","可愛い声で喘いでくれるんだろう？","まだまだご褒美はやらんぞ？","良く見れば、可愛い顔をしているぞ？");
...
ds_queue_enqueue(opp_words[? "s3_p4_0"],"[spr]hasu_bind_siro,1","素直でよろしい","ほおら、ご褒美だ！",0.2);
...
ds_queue_enqueue(opp_words[? "s3_p5_0"],"[spr]hasu_bind_siro,2",0.15,0.1,0.05,"ぐるぅう……!!!");
...
ds_queue_enqueue(opp_words[? "s3_p4_1"],"[spr]hasu_bind_siro,1","言っている事と身体の反応は違うようだが？","そら、これが欲しいのだろう……!",0.2);
...
ds_queue_enqueue(opp_words[? "s3_p5_1"],"[spr]hasu_bind_siro,2",0.15,0.1,0.05,"ぐるぅう……!!!");
ds_queue_enqueue(player_words[? "s3_p5_1"],"やぁぁ……");

opp_words[? "s3_p6_0"] = ds_queue_create(); //事後
player_words[? "s3_p6_0"] = ds_queue_create(); //事後
ds_queue_enqueue(opp_words[? "s3_p6_0"],"[spr]hasu_stand,0",0.1,"ふうう、今回も好かったぞ","まだまだ続けたいのではないか？ ふふふ",50);
...
ds_queue_enqueue(opp_words[? "s3_p6_1"],"[spr]hasu_stand,0",0.1,"ふうう、今回も好かったぞ","まだまだ続けたいのではないか？ ふふふ",50);
